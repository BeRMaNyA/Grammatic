...
module Grammatic
  ...
  module Grammar
    ...
    class Expense < Transaction
      def to_json
        super.merge({ :account => format_output(expense.account), :tags => format_output(expense.tags), :amount => format_output(expense.debit, :integer) })
      end
    end
    ...
    class Deposit < Transaction
      def to_json
        super.merge({ :account => format_output(deposit.account), :tags => format_output(deposit.tags), :amount => format_output(deposit.credit, :integer) })
      end
    end
    ...
    class Transfer < Transaction
      def to_json
        super.merge({ :from => format_output(transfer.from), :to => format_output(transfer.to), :ammount => format_output(transfer.amount, :integer) })
      end
    end
    ...
    class Exchange < Transaction
      def to_json
        super.merge({ :from => format_output(exchange.from), :to => format_output(exchange.to), :sell => format_output(exchange.sell, :integer), :buy => format_output(exchange.buy, :integer) })
      end
    end
  end
